...
setwd("C:/Data Projects/Datasets/ACS PUMS")
...
ACS_2022<-read.csv("usa_00011.csv")
str(ACS_2022)
...
# ACS_Sample<-ACS_2022[sample(nrow(ACS_2022), 5000), ]
# write.csv(ACS_Sample, "ACS_Sample.csv")


metro_labels<-read.csv("MET2013_Labels.csv")
str(metro_labels)
ACS_2022<-merge(ACS_2022, metro_labels, by = "MET2013", all.x=TRUE)

# Do the same for occupation labels.
occ_labels<-read.csv("Occ_Names.csv")
...
# The process can be reproduced for other occupations
